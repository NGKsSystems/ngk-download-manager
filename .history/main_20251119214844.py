...
class DownloadManagerGUI:
    def __init__(self, root):
        ...
        self.downloads_db = DownloadsDatabase()  # Persistent downloads database
        ...
        self.setup_ui()
        self.load_config()
        self.load_downloads_from_database()  # Load persisted downloads
        self.load_history()  # Load download history on startup
        
        # Note: Resume check temporarily disabled to prevent UI lockup
        # Will be re-enabled after fixing the state manager integration
        # self.root.after(500, self.check_resumable_downloads)
    def setup_ui(self):
    ...
    def setup_download_tab(self):
        ...
        # Add context menu to destination entry (copy only since it's readonly)
        self.setup_readonly_entry_context_menu(self.dest_entry)
    ...
    def setup_settings_tab(self):
        ...
        # Add context menu to HF token entry
        self.setup_entry_context_menu(self.hf_token_entry)
    ...
    def setup_readonly_entry_context_menu(self, entry_widget):
        """Setup right-click context menu for readonly entry widgets"""
        context_menu = tk.Menu(self.root, tearoff=0)
        ...
        # Add menu items (only copy and select all for readonly)
        context_menu.add_command(label="Copy", command=copy_text, accelerator="Ctrl+C")
        context_menu.add_command(label="Select All", command=select_all, accelerator="Ctrl+A")
    ...
    def update_progress(self, download_id, progress_info):
        ...
        if len(current_values) >= 6:
            # Clean filename by removing quotes for display and file creation
            raw_filename = progress_info.get('filename', current_values[1]) 
            clean_filename = raw_filename.replace('"', '').replace("'", '') if isinstance(raw_filename, str) else raw_filename
            current_values[1] = clean_filename
        ...
        # Parse progress percentage from string like "45.2%"
        progress_str = progress_info.get('progress', '0%')
        try:
            progress_num = float(progress_str.replace('%', ''))
        except:
            progress_num = 0
        
        # Update database with current progress
        self.downloads_db.update_download(download_id,
            progress=progress_num,
            speed=progress_info.get('speed', '0 B/s'),
            status=progress_info.get('status', 'downloading'),
            downloaded=progress_info.get('downloaded', 0),
            chunks=progress_info.get('chunks', 0)
        )
    ...
    def download_failed(self, download_id):
        ...
        # Update database
        self.downloads_db.update_download(download_id, status='failed')
    ...
    def load_downloads_from_database(self):
        """Load all persisted downloads from database"""
        try:
            # Get all downloads from database
            downloads = self.downloads_db.get_all_downloads()
            
            # Clear current display
            for item in self.downloads_tree.get_children():
                self.downloads_tree.delete(item)
            
            # Add each download to the tree
            for download in downloads:
                download_id = download.get('id')
                filename = download.get('filename', 'Unknown')
                url_type = download.get('type', 'Unknown')
            ...
            print(f"Error loading downloads from database: {e}")
    
    def check_resumable_downloads(self):
        """Check for partial downloads that can be resumed"""
        try:
            resumable = self.download_manager.get_resumable_downloads()
            
            if resumable and len(resumable) > 0:
                # Create a message showing resumable downloads
                msg = "Found incomplete downloads from previous sessions:\n\n"
                for dl in resumable[:3]:  # Limit to first 3
                    progress = dl.get('progress', 0)
                    filename = dl['filepath'].split(os.sep)[-1] if os.sep in dl['filepath'] else dl['filepath']
                    msg += f"• {filename}\n"
                    msg += f"  {progress:.1f}% complete\n\n"
                
                msg += "Would you like to resume these downloads?"
                
                try:
                    if messagebox.askyesno("Resume Downloads", msg):
                        for dl in resumable:
                            # Add URL to download list for resuming
                            self.url_var.set(dl['url'])
                            # Auto-start download
                            self.root.after(500, self.start_download)
                            break  # Only resume first one
                except:
                    pass
        except Exception as e:
            pass  # Silently ignore if no resumable downloads
