...
class YouTubeDownloader:
    ...
    def download(self, url, destination, progress_callback=None, extract_audio=False, auto_quality=True, quality="best"):
        ...
        try:
            ...
            # Configure yt-dlp options with quote-free template
            # Use yt-dlp's sanitization to remove quotes while keeping spaces
            clean_template = os.path.join(destination, "%(uploader|)s - %(title|)s.%(ext)s")
            ydl_opts = {
                ...
                'writethumbnail': True,
                'writeinfojson': True,
                'restrictfilenames': False,  # Keep spaces but we'll clean quotes manually
                'continue_dl': True,  # Resume incomplete downloads
                'part': True,  # Keep partial files for resume
                'retries': 3,  # Retry failed downloads
                'fragment_retries': 3,  # Retry failed fragments
                'ignoreerrors': False,  # Don't ignore errors
                'postprocessors': [],  # Will add specific postprocessors based on download type
                # Custom sanitization
                'outtmpl_na_placeholder': '',  # Remove N/A placeholders
            }
            
            # Configure quality settings
            if extract_audio:
                # Add audio-specific postprocessors to existing ones
                ...
                ydl_opts['postprocessors'].extend(audio_postprocessors)
                ydl_opts.update({
            ...
            # Clean filenames after download
            self._clean_downloaded_filenames(destination, info)
            
            # Clean up separate metadata and thumbnail files if audio extraction was used
            if extract_audio:
                self._cleanup_metadata_files(destination, self.current_filename)
    ...
    def download_with_format(self, url, destination, format_id, progress_callback=None):
        ...
        try:
            ...
            ydl_opts = {
                'outtmpl': os.path.join(destination, '%(uploader)s - %(title)s.%(ext)s'),
                'progress_hooks': [self._progress_hook],
                'format': format_id,
                'writethumbnail': True,
                'writeinfojson': True,
                'restrictfilenames': False,  # Keep spaces in filenames
                'continue_dl': True,  # Resume incomplete downloads
                'part': True,  # Keep partial files for resume
                'retries': 3,  # Retry failed downloads
                'fragment_retries': 3,  # Retry failed fragments
    ...
    def _clean_downloaded_filenames(self, destination, info):
        """Clean filenames after download to remove quotes and problematic characters"""
        try:
            # Get the expected uploader and title
            uploader = info.get('uploader', 'Unknown')
            title = info.get('title', 'Unknown')
            
            # Look for files starting with uploader name
            prefix = f"{uploader} - "
            ...
            for filename in os.listdir(destination):
                ...
                # Check if this file matches our download (starts with "uploader - ")
                if filename.startswith(prefix):
                    ...
                    if clean_name != name:
                        ...
                        if not os.path.exists(new_filepath):
                            ...
                            print(f"Cleaned filename: {filename} -> {new_filename}")
                        
        except Exception as e:
            print(f"Warning: Could not clean filenames: {e}")
