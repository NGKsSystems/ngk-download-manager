...
class FilenameCleanerPP(yt_dlp.postprocessor.PostProcessor):
    """Custom postprocessor to clean filenames after download"""
    
    def __init__(self, downloader=None, sanitizer=None):
        super().__init__(downloader)
        self.sanitizer = sanitizer
    
    def run(self, info):
        """Clean the filename after download"""
        if 'filepath' in info:
            old_path = info['filepath']
            if os.path.exists(old_path):
                ...
                # Clean the name part
                if self.sanitizer:
                    clean_name = self.sanitizer(name)
                    ...
                    if old_path != new_path and not os.path.exists(new_path):
                        try:
                            ...
                            self.to_screen(f'Cleaned filename: {old_filename} -> {new_filename}')
                        except OSError as e:
                            self.to_screen(f'Could not rename file: {e}')
    ...
    def download(self, url, destination, progress_callback=None, extract_audio=False, auto_quality=True, quality="best"):
        ...
        try:
            ...
            ydl_opts = {
                ...
                'postprocessors': [FilenameCleanerPP(sanitizer=self.sanitize_filename)],  # Always clean filenames
            }
