...
class DownloadManagerGUI:
    ...
    def _handle_youtube_download(self, url, destination):
        ...
        # Show progress dialog while getting video info
        progress_dialog = ProgressDialog(self.root, "Getting video information...")
        
        def get_video_info():
            try:
                progress_dialog.update_status("Fetching video details...")
                video_info = self.youtube_downloader.get_video_info(url)
                
                self.root.after(0, progress_dialog.close)
                
                if video_info:
                    self.root.after(0, self._show_quality_dialog, url, destination, video_info)
                else:
                    self.root.after(0, lambda: messagebox.showerror("Error", "Could not get video information"))
                    
            except Exception as e:
                self.root.after(0, progress_dialog.close)
                self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to get video info: {str(e)}"))
        
        threading.Thread(target=get_video_info, daemon=True).start()
    
    def _show_quality_dialog(self, url, destination, video_info):
        """Show quality selection dialog for YouTube"""
        dialog = QualitySelectionDialog(self.root, video_info, self.youtube_downloader)
    ...
    def _youtube_download_worker(self, download_id, url, destination, options):
        """Worker for YouTube downloads with format selection"""
        try:
            ...
            if options.get('format_id'):
                result = self.youtube_downloader.download_with_format(
                    url, destination, options['format_id'], progress_callback
                )
            else:
                result = self.youtube_downloader.download(
                    url, destination, progress_callback,
                    extract_audio=options.get('audio_only', False),
                    auto_quality=True
                )
    ...
    def update_progress(self, download_id, progress_info):
        ...
        # Debug logging
        print(f"DEBUG: update_progress called for download {download_id}")
        print(f"DEBUG: progress_info = {progress_info}")
        
        # Update treeview
        current_values = list(self.downloads_tree.item(item_id)['values'])
        print(f"DEBUG: current_values before = {current_values}")
        ...
        if len(current_values) >= 6:
            ...
            print(f"DEBUG: current_values after = {current_values}")
            self.downloads_tree.item(item_id, values=current_values)
            
            # Verify update
            verify_values = self.downloads_tree.item(item_id)['values']
            print(f"DEBUG: verified values = {verify_values}")
    ...
    def download_completed(self, download_id, result=None):
        ...
        print(f"DEBUG: download_completed - download_id={download_id}, actual_filename={actual_filename}")
