...
class DownloadManagerGUI:
    ...
    def _handle_youtube_download(self, url, destination):
        ...
        # Title
        ttk.Label(dialog, text="Select Download Quality:", font=("Arial", 12, "bold")).pack(pady=15)
        
        # Quality selection
        quality_var = tk.StringVar(value="auto")
        
        for text, value in quality_options:
            ttk.Radiobutton(dialog, text=text, variable=quality_var, value=value).pack(anchor=tk.W, padx=30, pady=5)
        ...
        ttk.Button(button_frame, text="Download", command=on_download).pack(side=tk.LEFT, padx=10)
